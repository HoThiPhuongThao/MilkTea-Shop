...
<script>
    function searchProduct() {
        ...
        if (searchInput) {
            window.location.href = 'tea.php?search=' + encodeURIComponent(searchInput);
        }
    }
</script>
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT * FROM product WHERE category_id = 113 AND title LIKE '%$searchQuery%' LIMIT $start, $productsPerPage";
    $countSql = "SELECT COUNT(*) as total FROM product WHERE category_id = 113 AND title LIKE '%$searchQuery%'";
} else {
    $sql = "SELECT * FROM product WHERE category_id = 113 LIMIT $start, $productsPerPage";
    $countSql = "SELECT COUNT(*) as total FROM product WHERE category_id = 113";
}
